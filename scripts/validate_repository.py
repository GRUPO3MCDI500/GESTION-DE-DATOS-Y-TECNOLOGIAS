...
def parse_event(path: Path) -> list[tuple]:
    ...
    with path.open(encoding="utf-8-sig") as fh:
        ...
        if header != "cod_evento,evento,deporte_id":
            fail("Encabezado inesperado en evento.csv")
        for line_number, line in enumerate(fh, start=2):
            ...
            if len(values) != 3:
                fail(f"evento.csv:{line_number} no se pudo interpretar")
...
with (DATA / "equipo.csv").open(encoding="utf-8-sig", newline="") as fh:
    teams = list(csv.DictReader(fh))
if len(teams) != 1184 or set(teams[0]) != {"id", "equipo", "sigla"}:
    fail("equipo.csv no cumple la estructura esperada")

events = parse_event(DATA / "evento.csv")
if len(events) != 765 or {row[0] for row in events} != set(range(1, 766)):
    fail("evento.csv debe contener los eventos 1 a 765")

with (DATA / "resultados.csv").open(encoding="utf-8-sig", newline="") as fh:
...
if set(results[0]) != {"resultado_id", "medalla", "deportista_id", "juego_id", "evento_id"}:
    fail("resultados.csv no cumple la estructura esperada")

with (DATA / "juegos.json").open(encoding="utf-8") as fh:
    games = json.load(fh)
if len(games) != 51:
    fail("juegos.json debe contener 51 ediciones")
if set(games[0]) != {"juego_id", "ano", "temporada", "ciudad"}:
    fail("juegos.json no cumple la estructura esperada")
